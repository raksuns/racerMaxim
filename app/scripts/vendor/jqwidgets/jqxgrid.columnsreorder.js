...
(function ($) {
    $.extend($.jqx._jqxGrid.prototype, {
        ...
        setcolumnindex: function (datafield, index, refresh, eventskip) {
            var column = this.getcolumn(datafield);
			...
			// 컬럼이동후 cells cache를 지우지 않는 버그 수정. - ysjung(2014/03/21)
        ...
        _handlecolumnsreorder: function () {
            ...
            this.addHandler($(document), mousemove, function (event) {
                ...
                if (self.reordercolumn != null) {
					...
					// y축은 변경되지 않도록 수정 - ysjung(2014/03/20)
					var topposition = hosttop;
                    ...
                    if (candrop) {
						...
						// header를 벗어나도 drop이 되도록 수정 - ysjung(2014/03/20)
                            ...
                            if (dropindex != -1) {
                                ...
                                if (index >= minIndex) {
                                    ...
                                    if (targetcolumn != undefined) {
										...
										// reorder가 되지 않아야 할 column에 대한 처리 추가 - ysjung(2014/03/20)
										if(!targetcolumn.columnreorder) {
											if (self.columnsdropline != null) {
                                        ...
                                        if (self.columngroups) {
	                                        ...
	                                        // group컬럼에 대해서도 옮기도록 수정 - ysjung(2014/03/26)
                                            if (target.columngroup != self.getcolumn(datafield).columngroup) {
	                                            var cg, ds, i, j;
	                                            if (oldindex < columnindex) {
		                                            // before 위치(왼쪽)에서 오른쪽으로 옮길때
		                                            if (dropindex.position == 'before') {
			                                            if(target.columngroup) {
				                                            for(i=columnindex - 1; i<self.columns.records.length; i++) {
					                                            if(self.columns.records[i].columngroup !== target.columngroup) {
						                                            cg = self.getcolumn(datafield).columngroup;
						                                            if(cg) {
							                                            ds = [];
							                                            for(j=0;j<self.columns.records.length;j++) {
								                                            if(self.columns.records[j].columngroup === cg) {
									                                            ds.push(self.columns.records[j].datafield);
								                                            }
							                                            }

							                                            // event를 첫번째만 보내도록 함.
							                                            $.each(ds, function(index, value) {
								                                            self.setcolumnindex(value, i - 1, true, index !== 0);
							                                            });
						                                            }
						                                            else {
							                                            self.setcolumnindex(datafield, i - 1);
						                                            }
						                                            break;
					                                            }
				                                            }
				                                            ...
				                                            if(cg) {
					                                            ...
					                                            // event를 첫번째만 보내도록 함.
					                                            $.each(ds, function(index, value) {
						                                            self.setcolumnindex(value, columnindex - 1, true, index !== 0);
					                                            });
				                                            }
			                                            }
		                                            }
			                                            ...
			                                            if(target.columngroup) {
				                                            ...
				                                            if(cg) {
					                                            ...
					                                            // event를 첫번째만 보내도록 함.
					                                            $.each(ds, function(index, value) {
						                                            self.setcolumnindex(value, i, true, index !== 0);
						                                            i++;
					                                            });
				                                            }
			                                            }
        ...
        _createreordercolumn: function (column, position, hascolumnsdropline) {
            ...
            columnclone.css('opacity', '0.5');
//			columnclone.css('background', 'darkgray');
//			columnclone.css('font-size', '13px');
            ...
            if (hascolumnsdropline != undefined && hascolumnsdropline) {
				...
				// 두께를 조금 두껍게 조정 - ysjung(2014/03/20)
                me.columnsdropline.width(5);
        ...
        _handlecolumnsdragreorder: function (record, column) {
            ...
            this.addHandler(column, mousedown, function (event) {
				// pinned column을 드래그했을때 reorder가 안되도록 수정 - ysjung(2014/03/25)
				if (record.pinned) {
					return true;
				}
            ...
            this.addHandler(column, mousemove, function (event) {
                ...
                if (me.dragmousedown) {
					// reorder가 되지 않아야 할 column에 대한 처리 추가 - ysjung(2014/03/20)
					if(!record.columnreorder) {
						me.dragmousedown = false;
						return true;
					}
