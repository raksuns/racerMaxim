...
Model.prototype.bundle = function (cb) {
	...
	root.whenNothingPending(function finishBundle() {
		...
		var bundle = {
			queries: root._queries, contexts: root._contexts, refs: root._refs, refLists: root._refLists, fns: root._fns
			// Get the filters before removing their computed values below
			, filters: root._filters.toJSON(), nodeEnv: process.env.NODE_ENV
		};
		stripComputed(root);
...
function serializeCollections(root) {
	...
	for (var collectionName in root.collections) {
		...
		for (var id in collection.docs) {
			...
			{
				v: shareDoc.version, data: shareDoc.snapshot, type: shareDoc.type && shareDoc.type.name
			} :
				doc.snapshot;
		}
	}
